...
export class Level {
  constructor() {
    ...
    this.nextTetrominoes = Array.from(Array(6), () =>
      this.tetrominoSource.getNext()
    );
    this.nextTetromino();
  ...
  step() {
    ...
    if (this.clearAnimation) {
      if (this.clearAnimation.done) {
        ...
        this.scaredTetrominoControllers = this.getScaredTetrominoControllers(
          this.fleeingTetrominoesCandidates
        );
    ...
    if (currentScore !== previousScore) {
      this.scoreAnimations.push(
        new ScoreAnimation(currentScore - previousScore)
      );
  ...
  updateControllers() {
    let disableControls =
      this.scaredTetrominoControllers.size > 0 || this.endAnimation;
  ...
  render() {
    ...
    if (!this.paused && !this.endAnimation) {
    ...
    if (!this.paused && !this.endAnimation && this.heldTetromino) {
    ...
    drawAt(-10, 50, () => {
      if (this.clearStreak > 1) {
        ...
        drawBoldText(
          `${this.clearStreak < 10 ? " " : ""}${this.clearStreak - 1}`,
          -5,
          7,
          2
        );
  ...
  getTSpin() {
    if (
      !(this.currentTetromino instanceof TetrominoT) ||
      this.controller.lastMove !== ACTION_ROTATE
    ) {
      return false;
    }
    ...
    let emptyCorners = !corners[0] + !corners[1] + !corners[2] + !corners[3];
  ...
  startClearingLines(rowsToClear) {
    ...
    const scaredTetrominoes = this.getScaredTetrominoes(
      rowsToClear,
      effectedTetrominoes
    );
  ...
  renderTetrominoEyes(tetromino) {
    ...
    if (tetromino.eyesIndex === 0) {
      Graphics.fillRect(
        x1 - 1 + tetromino.eyeDirection[0],
        y - 1 + tetromino.eyeDirection[1],
        2,
        2
      );
      Graphics.fillRect(
        x2 - 1 + tetromino.eyeDirection[0],
        y - 1 + tetromino.eyeDirection[1],
        2,
        2
      );
  ...
  getTimeText() {
    ...
    return `${zeroPad(hours, 2)}:${zeroPad(minutes % 60, 2)}:${zeroPad(
      seconds % 60,
      2
