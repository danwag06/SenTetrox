...
export let Input = {
  ...
  preUpdate() {
    if (Input.gamepad) {
      ...
      update(ROTATE_CW, Input.isPressed(1) || Input.isPressed(3));
      update(ROTATE_CCW, Input.isPressed(0) || Input.isPressed(2));
      update(
        HOLD,
        Input.isPressed(4) ||
          Input.isPressed(5) ||
      ...
      update(
        PAUSE,
        Input.isPressed(8) || Input.isPressed(9) || Input.isPressed(10)
      );
    }
  },

  postUpdate() {
    Object.entries(Input.current).forEach(([key, value]) => {
      Input.previous[key] = value;
    });
  },
